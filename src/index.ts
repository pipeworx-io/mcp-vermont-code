...
/**
 * Vermont Statutes Annotated — state statutes by citation.
 *
 * Vermont's section URL needs a CHAPTER that the citation does not carry:
 * "13 V.S.A. § 2301" lives at /statutes/section/13/053/02301. Nothing in
 * "13-2301" says chapter 53.
 *
 * Rather than bake a mapping table that would rot, the chapter is resolved
 * from Vermont's own title index, which publishes the ranges:
 *
 *   Chapter 053 : Homicide   Contains: §§ 2301 - 2311
 *
 * One extra request, and the answer stays correct when Vermont renumbers.
 * That is the difference between a lookup and a guess.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Vermont Statutes Annotated');
}

const BASE = 'https://legislature.vermont.gov/statutes';
const UA = 'pipeworx-mcp-vermont-code/1.0 (+https://pipeworx.io)';

/** Numeric part of a Vermont section, ignoring any letter suffix. */
function num(s: string): number {
  return parseInt(s.replace(/[^0-9].*$/, ''), 10);
}

async function chapterFor(title: string, section: string): Promise<string | null> {
  const res = await pwFetch(`${BASE}/title/${encodeURIComponent(title)}`, {
    headers: { 'User-Agent': UA },
  });
  if (!res.ok) return null;
  const text = statuteText(await readStatuteResponse(res));
  const want = num(section);
  // "Chapter 053 : Homicide Contains: §§ 2301 - 2311"
  const re = /Chapter\s+(\d+)\s*:[^§]*§+\s*([0-9A-Za-z]+)\s*-\s*([0-9A-Za-z]+)/g;
  for (let m = re.exec(text); m; m = re.exec(text)) {
    if (want >= num(m[2]) && want <= num(m[3])) return m[1];
  }
  return null;
}
...
const tools: McpToolExport['tools'] = [{
  ...
  inputSchema: {
    type: 'object',
    properties: {
      title: { type: 'string', description: 'Vermont title number, e.g. "13" (crimes), "23" (motor vehicles), "15" (domestic relations).' },
      section: { type: 'string', description: 'Section as cited, e.g. "2301" (murder) or "1201" (DUI).' },
    },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'vt_statute') throw new Error(`Unknown tool: ${name}`);
  const title = String(args.title ?? '').trim().replace(/[^0-9A-Za-z]/g, '');
  const section = String(args.section ?? '').trim().replace(/\s+/g, '');
  if (!title || !/^\d+[a-z]?$/i.test(section)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.title} V.S.A. ${args.section}" is not a citation this can resolve.`,
      hint: 'Vermont citations are title and section, e.g. title "13" section "2301" (murder).' };
  }

  const chapter = await chapterFor(title, section);
  if (!chapter) {
    return { found: false, reason: 'section_not_found', title, section,
      message: `No chapter in Vermont title ${title} covers section ${section}.`,
      hint: 'The chapter is resolved from Vermont’s own title index; a section outside every published range does not exist in that title.' };
  }

  const url = `${BASE}/section/${title}/${chapter}/${section.padStart(5, '0')}`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (!res.ok) {
    return { found: false, reason: 'section_not_found', title, section, chapter,
      message: `No section ${section} in Vermont title ${title} chapter ${chapter} (HTTP ${res.status}).` };
  }
  const full = statuteText(await readStatuteResponse(res));
  // Anchor on the section symbol: the page opens with the site's skip-links
  // and breadcrumb, so converting it whole puts "Skip to content Skip to
  // navigation" where the statute belongs.
  const at = full.indexOf(`§ ${section}.`);
  const text = at === -1 ? full : full.slice(at).trim();
  const heading = at === -1 ? null
    : (text.match(/^§\s*[\dA-Za-z]+\.\s*([^\n]{2,140})/) ?? [])[1]?.trim() ?? null;
  return {
    citation: `${title} V.S.A. § ${section}`, title, section, chapter, heading,
    text, text_chars: text.length, url,
    source: 'Vermont General Assembly (legislature.vermont.gov), keyless',
    attribution: 'Vermont statutes are public record.',
  };
}
